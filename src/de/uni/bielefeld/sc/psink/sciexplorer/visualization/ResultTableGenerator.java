 ...
 */
package de.uni.bielefeld.sc.psink.sciexplorer.visualization;
...
import de.uni.bielefeld.sc.psink.sciexplorer.utility.HTMLUtility;
...
public class ResultTableGenerator {
	...
	public static QueryResult generateTable(List<AbstractStatement> statements) {
		...
		for (List<String> resultRow : results.getData()) {
			publication = resultRow.get(0);
			publicationResultMap.putIfAbsent(publication, new LinkedList());
			publicationResultMap.get(publication).add(resultRow.get(1));
		}
		...
		return new QueryResult(VARIABLES, table);
	}
	...
	private static Map<String, Map<String, List<String>>> generateTripleMap(QueryResult triples) {
		...
		for (List<String> triple : triples.getData()) {
			s = triple.get(0);
			if (!tripleMap.containsKey(s)) {
				tripleMap.put(s, new LinkedHashMap());
			}
			p = optimizeValue(triple.get(1));
